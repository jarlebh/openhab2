...
/**
 * {@link TelldusCoreBridgeHandler} is the handler for a MAX! Cube and connects it
 * to the framework. All {@link MaxDevicesHandler}s use the
 * {@link TelldusCoreBridgeHandler} to execute the actual commands.
 *
...
public class TelldusCoreBridgeHandler extends BaseBridgeHandler
    ...
    public void onRequest(TellstickSensorEvent newEvent) {
        ...
        logger.info("Sensor Event for " + device + " event " + newEvent);
        if (device == null) {
            ...
            if (!currentValue.equals(newEvent.getData()) || useSensor.getTimeStamp().before(compdate.getTime())) {
                ...
                logger.info("Ignored update " + newEvent);
            }
        }
    }
